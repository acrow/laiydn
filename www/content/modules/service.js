  ...
  Spin.prototype.show  = function(info){
    ...
    if(this.count === 1){//it hasn't been started, yet.
            this.modalInstance = $modal.open({
              template: '<div class="text-center" style="color:white;"><img src="/view/weixin/img/loading.gif" /><br><p>' + info + '</p></div>',
...
laiydApp.factory("prompt", ['$modal', function($modal) {
  var Dialog = function(){};
  Dialog.prototype.count = 0;
  Dialog.prototype.show  = function(info, callback){
  	if (!info) {
  		info = "确定要继续吗？";
  	}
    this.modalInstance = $modal.open({
